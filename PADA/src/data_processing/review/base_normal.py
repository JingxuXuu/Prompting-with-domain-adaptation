...
class ReviewDataProcessor_normal:
    ...
    def load_data(self) -> Dict[str, Dict[str, List[Union[str, List[str], List[int]]]]]:
        return {split: self.read_data_from_file(mode=split) for split in ReviewDataProcessor_normal.ALL_SPLITS}

    def get_split_data(self, split: str) -> Dict[str, List[Union[str, List[str], List[int]]]]:
        assert split in ReviewDataProcessor_normal.ALL_SPLITS
...
class ReviewDataset_normal(Dataset):
    def __init__(self, split: str, data_processor: ReviewDataProcessor_normal, tokenizer: T5TokenizerFast, max_seq_len: int):
        self.split = split
