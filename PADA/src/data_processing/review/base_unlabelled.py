...
class ReviewunlabelledDataProcessor:
    ...
    def read_data_from_file(self, mode: str = 'train') -> Dict[str, List[Union[str, List[str], Tuple[int]]]]:
        ...
        if mode == 'dev':
            ...
            for domain_idx, domain in enumerate(domains):
                data_path = Path(self.data_dir) / "amazon_review" / domain / "UN.txt"

                data_tree = ET.parse(data_path)
                data_root = data_tree.getroot()
                for rev in data_root.iter('review'):
    ...
    def load_data(self) -> Dict[str, Dict[str, List[Union[str, List[str], List[int]]]]]:
        return {split: self.read_data_from_file(mode=split) for split in ReviewunlabelledDataProcessor.ALL_SPLITS}


    def get_split_data(self, split: str) -> Dict[str, List[Union[str, List[str], List[int]]]]:
        assert split in ReviewunlabelledDataProcessor.ALL_SPLITS
...
class ReviewunlabelledDataset(Dataset):
    def __init__(self, split: str, data_processor: ReviewunlabelledDataProcessor, tokenizer: T5TokenizerFast, max_seq_len: int):
        self.split = split
