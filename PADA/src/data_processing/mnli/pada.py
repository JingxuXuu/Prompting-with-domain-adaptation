...
from src.data_processing.mnli.base_unlabelled import MNLIunlabelledDataProcessor
from src.utils.constants import DATA_DIR, EXP_DIR
import torch


class MNLIPadaDataProcessor(MNLIunlabelledDataProcessor):
    ...
    def __init__(self, src_domains: List[str], trg_domain: str, data_dir: Union[str, Path], experiment_dir: Union[str, Path]):
        ...
        for split in MNLIunlabelledDataProcessor.ALL_SPLITS:
    ...
    def _read_drfs(self, mode: str = 'train'):
        ...
        for domain in domains:
            prompts_path = Path(self.experiment_dir) / "mnli" / self.trg_domain / "prompt_annotations" / domain
...
class MNLIPadaDataset(Dataset):

    DOMAIN_PROMPT = "domain"
    MNLI_PROMPT = "mnli"
    DRF_DELIMITER = ", "

    def __init__(self, split: str, data_processor: MNLIPadaDataProcessor, tokenizer: T5TokenizerFast,
    ...
    def _build_training_tokenized_data(data):
        ...
        for i in range(len(data["example_id"])):
            drfs_str = f"{data['domain_label'][i]} - " + f"{MNLIPadaDataset.DRF_DELIMITER.join(data['drfs'][i])}:"
            prompt = f"{MNLIPadaDataset.DOMAIN_PROMPT}: "
            domain_data["input_str"].append(prompt + "premise:" + data["input_str"][i][0] + " hypothesis: " + data["input_str"][i][1])
            ...
            prompt = f"{drfs_str}: "
            drf_data["input_str"].append(prompt + "premise:" + data["input_str"][i][0] + " hypothesis: " + data["input_str"][i][1])
