...
class MNLIDataProcessor:
    ...
    def read_data_from_file(self, mode: str = 'train') -> Dict[str, List[Union[str, List[str], Tuple[int]]]]:
        ...
        if mode != 'test':
            ...
            for domain_idx, domain in enumerate(domains):
                data_path = Path(self.data_dir) / "mnli_data" /domain / "train.txt"
    ...
    def load_data(self) -> Dict[str, Dict[str, List[Union[str, List[str], List[int]]]]]:
        return {split: self.read_data_from_file(mode=split) for split in MNLIDataProcessor.ALL_SPLITS}

    def get_split_data(self, split: str) -> Dict[str, List[Union[str, List[str], List[int]]]]:
        assert split in MNLIDataProcessor.ALL_SPLITS
...
class MNLIDataset(Dataset):
    def __init__(self, split: str, data_processor: MNLIDataProcessor, tokenizer: T5TokenizerFast, max_seq_len: int):
        self.split = split
    ...
    def _init_tokenized_data(split, data_processor, tokenizer, max_seq_len):
        ...
        for sentence_pair in data["input_str"]:
            sentence_lst.append("premise: "+sentence_pair[0]+ tokenizer.eos_token + " hypothesis: " +sentence_pair[1])
