...
class RumorDataProcessor:
    ...
    def read_data_from_file(self, mode: str = 'train') -> Dict[str, List[Union[str, List[str], Tuple[int]]]]:
        ...
        lengthdic = {"germanwings-crash": 469, "charliehebdo": 2079, "sydneysiege": 1221, "ottawashooting": 890,"ferguson": 1143}
        for domain_idx, domain in enumerate(domains):
            if mode != 'test':
                data_path = Path(self.data_dir) / "rumor_data" / domain / "train.txt"
                ...
                for i in range(len(dic["input_str"])):
...
def test_RumorDataProcessor():
    ...
    for split in RumorDataProcessor.ALL_SPLITS:
        print(split, len(data_processor.data[split]["input_str"]))
        print()
...
class RumorDataset(Dataset):
    ...
    def __len__(self):
        return len(self.tokenized_data["input_str"])
