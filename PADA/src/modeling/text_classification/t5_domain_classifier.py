...
from src.data_processing.mnli.base import MNLIDataProcessor
from src.data_processing.review.base import ReviewDataProcessor
from src.data_processing.review.base_unlabelled import ReviewunlabelledDataProcessor
...
class T5DomainClassifier(LightningModule):
    LOSS_IGNORE_ID = -100
    ...
    def __init__(self,
        ...
        self.hparams.num_labels = 4
        self.loss_fn = CrossEntropyLoss(ignore_index=T5DomainClassifier.LOSS_IGNORE_ID)
        self.model = T5ForConditionalGeneration.from_pretrained(self.hparams.t5_model_name,output_hidden_states=True)
        self.classifier = CnnClassifier(num_labels=self.hparams.num_labels,
                                        hidden_size=self.model.config.hidden_size,
                                        max_seq_length=self.hparams.max_seq_len)
        self.eval_metric_scorer = T5DomainClassifier._init_eval_metric_scorer(self.hparams.eval_metrics)
    ...
    def forward(self, input_ids: LongTensor, attention_mask: LongTensor, labels: LongTensor, **kwargs):
        ...
        cls_logits = self.classifier(encoder_outputs)[0]
    ...
    def training_step(self, batch: BatchEncoding, batch_idx: int) -> Tensor:
        ...
        Returns:
            Tensor of shape (1,) with the loss after the forward pass.
        """
        loss = self._forward_step(batch["input_ids"], batch["attention_mask"], batch["domain_label_id"])["loss"]
