...
def preproc(args):
   ...
   """
    ...
    for src in domains:
        ...
        path=("/home/ubuntu/efs/prompt/PADA")
        os.chdir(path)
        for domain in domains:
            ...
            if data_type == "review":
                ...
                for rev in pos_root.iter('review'):
                    num_sample += 1
                    train.append(rev.text)
                tmp_domain_labels = [dom_id]*num_sample
        ...
        if data_type == "review":
            source = []
            src_path = os.path.join("data", "amazon_review", src, "negative.parsed")
            neg_tree = ET.parse(src_path)
            neg_root = neg_tree.getroot()
            for rev in neg_root.iter('review'):

                source.append(rev.text)
            src_path = os.path.join("data", "amazon_review", src, "positive.parsed")
            pos_tree = ET.parse(src_path)
            pos_root = pos_tree.getroot()
            for rev in pos_root.iter('review'):
        ...
        for domain in domains:
            if data_type == "review":
                ...
                non_src_path = os.path.join("data", "amazon_review", domain, "negative.parsed")
                neg_tree = ET.parse(non_src_path)
                neg_root = neg_tree.getroot()
                for rev in neg_root.iter('review'):
                    non_source.append(rev.text)
                non_src_path = os.path.join("data", "amazon_review", domain, "positive.parsed")
                pos_tree = ET.parse(non_src_path)
                pos_root = pos_tree.getroot()
                for rev in pos_root.iter('review'):
                    non_source.append(rev.text)
            else:
...
def main():
    ...
    parser.add_argument("--dtype",
                        choices=['absa', 'rumor', 'mnli',"review"],
                        default='rumor',
                        type=str,
                        required=True,
                        help="The task name.")
