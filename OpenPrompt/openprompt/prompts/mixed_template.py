...
class MixedTemplate(Template):
    ...
    """
    registered_inputflag_names = ["soft_token_ids", "loss_ids", "shortenable_ids","fixed_token_ids"]
    ...
    def __init__(self,
                 ...
                 fixed_embed = None,
                 domain_classification_model = None
                ):
    ...
    def get_default_fixed_token_ids(self) -> List[int]:
        return self.fixed_token_ids
        ...
        **Note that ``"soft_id"`` should have index start from 1 but not 0**
        ...
        for d in self.text:
            if "soft" not in d and "soft_id" not in d:
                ...
                fixed_token_ids.append(0)
                
                continue
            if "soft" in d and "fixed_id" in d:
                
                text.append({'soft':""})
                soft_token_ids.append(0)
                fixed_token_ids.append(d["fixed_id"])
            ...
            if "soft_id" in d:
                ...
                if d["soft_id"] in idx_mp:
                    ...
                    fixed_token_ids.extend([0 for _ in range(len(id_list))])
        ...
        self.fixed_token_ids = fixed_token_ids
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        for i in range(np.shape(batch['fixed_token_ids'])[0]):
            for j in range(np.shape(batch['fixed_token_ids'])[1]):
                if batch['fixed_token_ids'][i][j] >0:
                    raw_embeds[i][j] = self.fixed_embed(input_ids=batch['input_ids'][i],attention_mask= batch['attention_mask'][i],domain_classification_model=self.domain_classification_model).output[batch['fixed_token_ids'][i][j]-1]
